...
/*		
		...
		.then(function(devices) {
		  devices.forEach(function(device) {
			...
			Webcam.camera.name = device.kind + ": " + device.label;
			Webcam.camera.id = device.deviceId;
			Webcam.cameraIDs.push(device.deviceId);
			Webcam.all_cameras.push(Webcam.camera);
			}
		  });
		})
		.catch(function(err) {
        ...
        try {
			console.log('Try MediaStreamTrack');
...
*/
	...
	// Test function that returnes an array of all VideoInput Devices
	get_cameras : function (){
		...
		return Webcam.cameraIDs;
	},
	
	attach_with_camera: function (elem, camera_nr)
	{
		navigator.mediaDevices.enumerateDevices()
		.then(function(devices) {
			//Push all devices to an Array
			Webcam.cameraIDs = [];
			devices.forEach(function(device) {
				if (device.kind === 'videoinput') {
				//console.log(device.kind + ": " + device.label +
			...
			Webcam.attach(elem, camera_nr);
			//console.log(Webcam.cameraIDs);
			//console.log(Webcam.cameraIDs[camera_nr]);					
		})
		.catch(function(err) {
		  console.log(err.name + ": " + err.message);
		});
	...
	attach: function(elem, camera_nr) {
		...
		if (this.userMedia) {
            ...
            */
            ...
            //if (Webcam.cameraIDs.length > 0){
                ...
                constraints = {
                    video: {
						...
						optional: [{sourceId: Webcam.cameraIDs[camera_nr]}]
                    },
                    audio: false
                };
			...
			function(err) {
				return self.dispatch('error', "Could not access webcam. " + err.name);
			});
		}
		else {
			// flash fallback
